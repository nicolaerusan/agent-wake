...
What helps, then? One design instinct covers most of it: **make every write path a capability, and make the hub the enforcement point** — it's the one place that runs *before* the expensive agent boots, so it's where emit credentials, rate limits, parking, and dead-letters all belong. Two more rules do disproportionate work. First, put *references* in events, not secrets — the thin-ping philosophy applied to your own payloads: carry a pointer, let the reader fetch with its own credentials, and the world-readable log stops being a confidentiality incident. Second, for expensive agents, wake in **two stages**: a cheap, tightly-sandboxed triage pass classifies pending events and only escalates to the tool-bearing agent when they match expected shapes. Notice what happened there — the defense against prompt injection and the defense against wake-storm economics turned out to be the *same mechanism*. In a protocol where invocation costs money and the receiver can be talked into things, security and cost control aren't separate chapters; they're both "be deliberate about what gets to wake the reasoning loop."
